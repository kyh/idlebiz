...
agent-browser eval 'location.hash = "#/ui"'   # office builder, no company needed
agent-browser screenshot /tmp/office.png
agent-browser close
pnpm dev:kill                   # tear the session down
```

Under headless automation Phaser's boot stalls (`document.hidden` never flips), so the game
canvas stays blank. Force it:
...
- **Office art and collision are independent sections of `office-design.json`.** After any
  layout edit run `pnpm -F @repo/desktop check:office` (already part of `pnpm verify`).
