...
export const newKeyring = async (): Promise<Keyring> => {
  ...
  return { privateKey: pair.privateKey, publicKey: Buffer.from(raw).toString("base64url") };
};
...
export const seal = async (recipientPublicKey: string, payload: Sealable): Promise<string> => {
  ...
  const ciphertext = await subtle.encrypt({ iv, name: "AES-GCM" }, key, plaintext);
...
export const open = async <T>(
  ...
  if (bytes.length <= RAW_KEY_BYTES + IV_BYTES) {
    return null;
  }
  ...
  try {
    ...
    const plaintext = await subtle.decrypt({ iv, name: "AES-GCM" }, key, ciphertext);
