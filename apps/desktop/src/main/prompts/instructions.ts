...
- Pushing code: push a committed branch with the push tool, which pushes it with the founder's own git credentials once they sign off. Your run has no git credentials and no ssh agent, so a plain \`git push\` has nothing to sign in with, and it is held for the founder anyway. Never put a login or token in a remote's URL.
...
- Secrets: the founder's keys stay with IdleBiz and never reach your environment; deploying, pushing and charging are tools that ask the founder first. Never print or commit a secret value, wherever you find one.
