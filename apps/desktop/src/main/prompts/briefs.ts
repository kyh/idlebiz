...
export const betMark = (bet: Bet): string =>
  bet.claim.metric === "users"
    ...
    : `It counts only captured USD charges tagged metadata[bet]=${bet.id}: charge for it only with create_payment_link and "bet":"${bet.id}", which sets that tag on every payment.`;
