...
import { betGoal, isFundable } from "@/shared/bets";
...
const TOOLS = {
  ...
  delegate: define(TOOL_SPECS.delegate, (ctx, { role, title, description, product, bet }) => {
    ...
    const betId = bet ?? (product === undefined ? run.betId : null);
    const funded = betId === null ? null : store.getBet(betId);
    if (bet !== undefined && (!funded || funded.companyId !== company.id || !isFundable(funded))) {
