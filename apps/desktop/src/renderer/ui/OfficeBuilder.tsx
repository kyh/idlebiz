...
function floorSrc(kind: string): string {
  return `/workspace-kit/office-tiles/floor-${kind}.png`;
}
function wallSrc(kind: string): string {
  return `/workspace-kit/office-tiles/wall-${kind}.png`;
}
...
export function OfficeBuilder() {
  ...
  useEffect(() => {
    let cancelled = false;
    void window.appBridge?.loadOfficeDesign().then((res) => {
        ...
        <div className="px-scroll grid min-h-0 flex-1 grid-cols-3 gap-1 overflow-y-auto p-2">
          {visibleItems.map((it) => {
            ...
            return (
              ...
              >
                {/* oxlint-disable-next-line next/no-img-element -- local pixel sprite */}
            ...
            {layout.floorZones.map((z) => (
              <div
                ...
                style={{
                  ...
                  height: z.h,
                  zIndex: 0,
                  backgroundImage: `url(${floorSrc(z.kind)})`,
                  backgroundRepeat: "repeat",
                  imageRendering: "pixelated",
                }}
              />
            ))}
            {layout.wallRects.map((w) => (
              <div
                key={`w-${w.kind}-${w.x}-${w.y}-${w.w}-${w.h}`}
                style={{
                  position: "absolute",
                  left: w.x,
                  top: w.y,
                  ...
                  height: w.h,
                  zIndex: 1,
                  backgroundImage: `url(${wallSrc(w.kind)})`,
                  backgroundRepeat: "repeat",
                  imageRendering: "pixelated",
                }}
              />
