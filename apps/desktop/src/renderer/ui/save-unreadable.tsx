  ...
  return (
    <div className="pointer-events-auto absolute inset-0 z-40 flex items-center justify-center bg-black/55 p-6">
      <div className="px-battle w-full max-w-lg p-4">
        <div className="text-base text-text">Your save can't be read</div>
        <div className="mt-1 text-sm leading-relaxed text-text-dim">
        ...
        <ul className="mt-3 space-y-2">
          {issues.map((issue) => (
            <li key={issue.path} className="px-inset p-2 text-xs">
              <div className="truncate text-text">{issue.path}</div>
              <div className="mt-0.5 text-text-dim">{issue.error}</div>
            </li>
          ))}
        </ul>
